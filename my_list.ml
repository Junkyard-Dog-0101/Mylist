...
let remove_assoc my_key my_list =
  let rec remove_assoc_aux my_key new_list = function
    | Empty -> new_list
    | Item ((key, value), tail) ->
      if key = my_key
      then append tail new_list
      else remove_assoc_aux my_key (Item ((key, value), new_list)) my_list
  in remove_assoc_aux my_key Empty my_list;;
